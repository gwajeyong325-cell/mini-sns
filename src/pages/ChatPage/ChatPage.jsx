  ...
  return (
    <Box sx={{ bgcolor: 'background.default', minHeight: '100vh', pb: 10 }}>
      ...
      <Box sx={{ px: 2, py: 1, display: 'flex', gap: 1 }}>
        <Chip label="전체" size="small" sx={{ bgcolor: 'primary.main', color: 'white', fontWeight: 600 }} />
        <Chip label="그룹" size="small" variant="outlined" sx={{ fontWeight: 600 }} />
        <Chip label="1:1" size="small" variant="outlined" sx={{ fontWeight: 600 }} />
      </Box>
      ...
      <Box sx={{ bgcolor: 'white', mx: 2, borderRadius: 3, border: '1px solid #e5e5ea', overflow: 'hidden' }}>
        {MOCK_ROOMS.map((room, index) => (
          <Box key={room.id}>
            ...
            {index < MOCK_ROOMS.length - 1 && <Divider sx={{ mx: 2 }} />}
          </Box>
        ))}
      </Box>
